...
def compute_metrics(eval_pred,all_length):
    ...
    for i in range(len(all_length)):
        ...
        t=labels[start:end]
        # The test set does not contain articles with only one sentence. 
        # The Pk value of these articles is 0, so the result needs to be divided by dataset_size instead of len(all_length).
